...
from mag_register import Ui_Mag_Register
import pymysql
import MySQLdb
import hashlib

class open_mag_register(QtWidgets.QWidget, Ui_Mag_Register): # 注意：在对象初始化的时候import的是Ui_Form的类对象
    def __init__(self, parent=None):
        super(open_mag_register, self).__init__(parent)
    ...
    def write_database(self):
        ...
        if ((password_1 == password_2) and (len(password_1) != 0) and (len(password_2) != 0)):
            ...
            query_repeat= "select * from manager where magname = '%s'" % (mname)
            try:
                # 执行SQL语句
                ...
                if flag_user == 0:
                    reply=QMessageBox.warning(self, "系统消息提示", "该管理员已注册，请登录！", QMessageBox.Yes | QMessageBox.No,
                                                  QMessageBox.Yes)
                    print(reply)
                else:
                    # SQL 插入语句
                    query_register='insert into manager(magname, password) values(%s, %s)'
                    magname=mname
                    password=pwd

                    # 加密
                    md5.update(password.encode('utf-8'))
                    values=(magname, md5.hexdigest())
                    ...
                    # 显示成功写入数据库的信息框
                    reply=QMessageBox.information(self, "系统消息提示", "管理员注册成功！", QMessageBox.Yes | QMessageBox.No,
                                                  QMessageBox.Yes)
                    print(reply)
            except:
                # 发生错误时回滚
