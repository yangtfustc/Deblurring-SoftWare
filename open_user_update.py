...
import sys
from user_update import Ui_User_Update
import pymysql
import MySQLdb
import hashlib

class open_user_update(QtWidgets.QWidget, Ui_User_Update): # 注意：在对象初始化的时候import的是Ui_Form的类对象
    def __init__(self, parent=None):
        super(open_user_update, self).__init__(parent)
    ...
    def user_update_password(self):
        ...
        if (len(username_input) == 0 or len(password_old) == 0 or len(password_new) == 0):
        ...
        if ((password_old != password_new) and (len(password_old) != 0) and (len(password_new) != 0)):
            ...
            # 先进行查询语句，找到该用户
            query_user="select * from user where username = '%s'" % (uname)
            try:
                # 执行SQL语句
                ...
                if flag_user == 1:
                    ...
                    # SQL更新
                    update_password="update user set password = '%s' where username = '%s'"  % (md5.hexdigest(), uname)
                    ...
                    # 显示成功写入数据库的信息框
                    reply=QMessageBox.information(self, "系统消息提示", "用户改密成功！", QMessageBox.Yes | QMessageBox.No,
                                                  QMessageBox.Yes)
                    print(reply)
            except:
                # 发生错误时回滚
